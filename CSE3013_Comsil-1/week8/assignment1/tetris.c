...
int recommend(NODE* root){
	...
	int y = -1;
	int score;
	int curBlockID = root->level;
	root->curBlockID = curBlockID;
	root->child = (NODE**)malloc(sizeof(NODE*)* 4*(WIDTH+2));
	...
	for (rotate = 0; rotate < 4; rotate++)
	{
		...
		{
			score = 0;
			if (CheckToMove(root->recField, nextBlock[curBlockID],rotate, 0, x) == 1)
			{
				root->child[idx] = (NODE*)malloc(sizeof(NODE));
				score = 0;
				...
				score += AddBlockToField(root->child[idx]->recField, nextBlock[curBlockID], rotate, y, x);
				score += DeleteLine(root->child[idx]->recField);
				if (root->child[idx]->level < VISIBLE_BLOCKS)
				{
					score += recommend(root->child[idx]);
				}
				if (max < score)
				{
					root->recBlockX = x;
