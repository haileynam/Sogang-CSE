...
unsigned char bin2char(string pass) {
...
int main(int argc, char *argv[]) {
    ...
    while(feof(input_fp) == 0) {
        ...
        for(int i = 0; i<(int)result.length(); i++) 
            coded_bin.push_back(result[i]);
        store = fgetc(input_fp);
    }
    //cout << "coded_bin: "<< coded_bin << endl;
    ...
    for(int j = 0; j< (int)coded_bin.length()/8; j++) {
        unsigned char temp = bin2char(coded_bin.substr(8*j, 8));
