    ...
    */
    ...
    while (getline(messagesFile, line))
    {
        ...
        { // 송신자로부터 수신자로의 경로가 존재하지 않는 경우
            cout << "from " << src << " to " << des << " cost infinite hops unreachable message " << message;
            messagesFile.close();
            return;
        }
        // hops 정보 저장
        ...
        // message 출력
        cout << "from " << src << " to " << des << " cost " << cost << " hops ";
...
int main(int argc, char *argv[])
{
    ...
    int change = 1;
    int line_num = 1;
    ofstream output("output_ls_teset.txt");
