...
bitmap_expand(struct bitmap*bitmap, size_t size)
{
	...
	bitmap->bit_cnt = size + old_size;
	bitmap->bits = realloc(bitmap->bits, byte_cnt(bitmap->bit_cnt)); // bitmap의 내용을 b에 넣기
	
	if (!bitmap->bits)
		return NULL;

	bitmap_set_multiple(bitmap, old_size, size, false); // bitmap 범위 밖의 부분 false로 채우기
	return bitmap;
}